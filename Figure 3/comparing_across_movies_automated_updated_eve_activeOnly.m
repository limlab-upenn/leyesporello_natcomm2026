...
rangeIDX = [67:81, 85:87]; % [76:78, 73:75, 79:81, 85:87] ;%[73:78, 79:81, 85:87];%[1:21, 25:27, 34:36, 43:45];%[1:3 10:15 19:21 25:27 34:36] ; %[10:15 19:21 25:27 34:36 43:45]; %[10:15]; %[1:21, 25:45];%[1:21];%[64:69];%[64:68];%[10 11 12 13 14 15]; %[1:24]; %all LacZ; %[28:33 37:42] %E1 & E2 %[10:15 25:27 34:36] t.yel vs. LacZ ; % enter embryo data to evaluate
...
for k =rangeIDX %1:(size(txtData,1)-1)
    ...
    active_outputM_list{k} = outputM_activeFramesOnly; %active_outputM or outputM_activeFramesOnly;
...
toPlot = [3 4];%[1:8]; % construct idx to plot
...
xStep_combined = reshape(xStep(:,[1 ]),[],1);
%output_combined = reshape(mRNA_active_output_Matrix_innerPercent(:,toPlot([1 2 3 4])), [], 1);
output_combined = reshape(mRNA_active_output_Matrix_innerPercent(:,[1 ]), [], 1);
...
toPlot = [1:4]; % construct idx to plot
...
xlabel_positions = [1 2 3 4];% [ 0    1.3   6.3    7.3    7.9    8.8   10 ];%[ 0    1.3   6.3    7.3    8.0    8.8   10   15]; % manually set position of boxcharts
...
xStep_combined = reshape(xStep(:,[2 4]),[],1);
onset_combined = reshape(onset_NC14prog_perConstruct_innerPercent(:,[2 4 ]), [], 1);
...
xlabel_positions =  [1:4]; %[ 0    1.3   6.3    7.3    7.9    8.8   10 ];%construct_lengths;;%[ 0    1.3   6.3    7.3    8.0    8.8   10   15]; % manually set position of boxcharts
...
frac_active_xGroup_combined = reshape(frac_active_xGroup(:,toPlot([2 4  ])),[],1);
frac_active_ONSET_combined = reshape(frac_time_active_sinceOnset_perConstruct(:,toPlot([2 4 ])), [], 1);
...
% hold on;
% boxchart(frac_active_xGroup(:, 5),frac_time_active_sinceOnset_perConstruct(:, 9), 'Notch', 'on', 'MarkerStyle', '.', 'MarkerColor','k', 'BoxFaceColor','magenta', 'BoxWidth', 0.7);
% hold on;
% boxchart(frac_active_xGroup(:, 7),frac_time_active_sinceOnset_perConstruct(:, 10), 'Notch', 'on', 'MarkerStyle', '.', 'MarkerColor','k', 'BoxFaceColor','magenta', 'BoxWidth', 0.7);
% 
% xticks(xStep(1,[5 7]))
% xticklabels(construct_names([5 7]))
% ylabel('Frac NC14 spent in ACTIVE state SINCE ONSET')
...
count = 1;
toEval = [4 5]; % list of construct indices to evaluate
for i = toEval
    pp = [1:3]+((i-1)*3);
...
meanAmp_xGroup_combined = reshape(frac_active_xGroup(:,toPlot([1 ])),[],1); % uses same matrix as onset time
meanAmp_combined = reshape(mean_amplitude_active_perConstruct(:,toPlot([1 ])), [], 1);
...
%     subplot(1, length(construct_names),count)
    plot(x_Int,mean_cum_active_interp{count}, 'Color', string(color_codes(count)), 'LineWidth',1.5); hold on;
...
% use line below to plot combo of all embryos
